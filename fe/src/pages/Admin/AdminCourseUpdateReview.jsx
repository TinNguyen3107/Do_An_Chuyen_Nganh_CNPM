...
function LMSVideoPlayer({ videoId, title }) {
  ...
  const changeSpeed = (s) => {
    setSpeed(s); playerRef.current?.setPlaybackRate(s); setShowSpeed(false);
  };
  ...
  return (
    ...
    >
      {/* YouTube iframe fills the entire box */}
      ...
      {ready && (
        <div className="absolute inset-0" style={{ cursor: 'pointer' }} onClick={togglePlay} />
      )}
        ...
        <div className="flex items-center gap-1 px-3 pb-2.5">
          ...
          <div className="flex items-center gap-1 group/vol">
            <button onClick={toggleMute} className="text-white hover:text-blue-400 transition-colors p-1.5 rounded-lg hover:bg-white/10">
              {muted || volume === 0 ? (
                ...
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>
              ) : (
          ...
          <div className="relative z-10">
            <button
              ...
              className="text-white/80 hover:text-white text-xs font-semibold px-2 py-1 rounded border border-white/20 hover:border-white/50 transition-colors"
            >
              {speed}×
            </button>
            {showSpeed && (
...
const REVIEW_LABEL = { pending: '⏳ Chờ duyệt', approved: '✅ Đã duyệt', rejected: '❌ Từ chối' };
    ...
    <div className="animate-fade-in max-w-7xl">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <p className="font-semibold text-white line-clamp-1">{course.title}</p>
                      ...
                      <td className="px-6 py-4">
                        {course.reviewedAt ? (
                          <p className="text-xs text-slate-400">
                            {new Date(course.reviewedAt).toLocaleString('vi-VN', {
      ...
      {showModal && selectedCourse && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-h-[90vh] overflow-y-auto w-full max-w-3xl">
            {/* Modal header */}
            <div className="sticky top-0 bg-slate-800 border-b border-slate-700 px-6 py-4 flex justify-between items-center rounded-t-2xl">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-bold text-white">Chi tiết bản cập nhật</h2>
                  <p className="text-xs text-blue-400 font-medium">{selectedCourse.title}</p>
                </div>
              </div>
              <button onClick={closeModal} className="text-slate-400 hover:text-white text-xl transition-colors">✕</button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-start gap-3 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                ...
                <div>
                  <p className="text-blue-400 font-semibold text-sm">Bản CẬP NHẬT</p>
                  <p className="text-blue-300/80 text-xs mt-0.5">
                    Khoá học đã được Admin xem xét lần cuối vào{' '}
                  ...
                  {selectedCourse.rejectionReason && (
                    <p className="text-slate-400 text-xs mt-1.5">
                      ...
                      <span className="text-red-300 font-medium">{selectedCourse.rejectionReason}</span>
                    </p>
                  )}
                </div>
              </div>
              ...
              {/* Course info */}
              <div>
                <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Thông tin khoá học</h3>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: 'Tiêu đề', value: selectedCourse.title },
                    { label: 'Giảng viên', value: selectedCourse.instructor?.name },
                    { label: 'Danh mục', value: selectedCourse.category?.name },
                    { label: 'Cấp độ', value: selectedCourse.level },
                    { label: 'Giá', value: selectedCourse.price === 0 ? 'Miễn phí' : `${Number(selectedCourse.price).toLocaleString('vi-VN')}đ` },
                    { label: 'Trạng thái review', value: REVIEW_LABEL[selectedCourse.reviewStatus] },
                  ].map(item => (
                    <div key={item.label} className="bg-slate-800 rounded-xl px-4 py-3">
                      <p className="text-xs text-slate-500 mb-1">{item.label}</p>
                      <p className="text-sm text-white font-medium">{item.value || '—'}</p>
                    </div>
                  ))}
                </div>
                <div className="mt-4 bg-slate-800 rounded-xl px-4 py-3">
                  <p className="text-xs text-slate-500 mb-1">Mô tả</p>
                  <p className="text-sm text-slate-300 leading-relaxed">{selectedCourse.description}</p>
                </div>
              </div>

              {/* Curriculum */}
              <div>
                <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Chương trình giảng dạy</h3>
                {selectedCourse.chapters?.length > 0 ? (
                  <div className="space-y-4">
                    {selectedCourse.chapters.map((ch, idx) => (
                      <div key={ch._id}>
                        <h4 className="font-bold text-slate-300 text-sm px-1 mb-2">
                        ...
                        {ch.lessons?.length > 0 ? (
                          <div className="space-y-2">
                            {ch.lessons.map((lesson, lIdx) => (
                              <div
                                key={lesson._id}
                                className="flex items-center justify-between bg-slate-800 hover:bg-slate-750 border border-slate-700 hover:border-blue-500/30 p-3.5 rounded-xl text-sm group transition-all"
                              >
                                <span className="font-medium text-slate-200 flex items-center gap-3 truncate pr-2">
                                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                                    lesson.type === 'video' ? 'bg-purple-500/20 text-purple-400' :
                                    lesson.type === 'text'  ? 'bg-blue-500/20   text-blue-400'   :
                                                              'bg-emerald-500/20 text-emerald-400'
                                  }`}>
                                    {lesson.type === 'video' && <span className="text-base">▶️</span>}
                                    {lesson.type === 'text'  && <span className="text-base">📝</span>}
                                    {lesson.type === 'quiz'  && <span className="text-base">❓</span>}
                                  </div>
                                  <span className="truncate">{lIdx + 1}. {lesson.title}</span>
                                </span>
                                <button
                                  onClick={() => setPreviewLesson(lesson)}
                                  className="text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1 text-xs font-bold shrink-0 border border-transparent hover:border-blue-500/30"
                                >
                                  ▶ Xem thử
                                </button>
                              </div>
                          ...
                          <p className="px-4 py-2 text-sm text-slate-600 italic">Chưa có bài học</p>
                        )}
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-slate-500 text-sm italic">Chưa có chương học nào</p>
                )}
              </div>
              ...
              {selectedCourse.reviewStatus === 'pending' && (
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">
                    Lý do từ chối <span className="text-slate-500 font-normal">(bắt buộc nếu từ chối)</span>
                  </label>
                  <textarea
                    value={rejectionReason}
                    onChange={e => setRejectionReason(e.target.value)}
                    placeholder="Nhập lý do từ chối bản cập nhật..."
                    rows={3}
                    className="w-full px-4 py-3 bg-slate-800 border border-slate-700 focus:border-red-500 text-white placeholder-slate-500 rounded-xl focus:outline-none focus:ring-1 focus:ring-red-500/30 text-sm resize-none transition-all"
                  />
                </div>
              )}
            </div>
            ...
            {/* Footer actions */}
            <div className="sticky bottom-0 border-t border-slate-700 bg-slate-800 px-6 py-4 rounded-b-2xl flex justify-end gap-3">
              <button
                onClick={closeModal}
                disabled={actionLoading}
                className="px-5 py-2 bg-slate-700 hover:bg-slate-600 text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-50"
              >
                Đóng
              </button>
              {selectedCourse.reviewStatus === 'pending' && (
                <>
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-5 py-2 bg-red-600 hover:bg-red-500 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-50"
                  >
                    {actionLoading ? 'Đang xử lý...' : '❌ Từ chối cập nhật'}
                  </button>
                  <button
                    onClick={() => handleApprove(selectedCourse._id)}
                    disabled={actionLoading}
                    className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold rounded-xl transition-colors shadow-lg shadow-emerald-500/20 disabled:opacity-50"
                  >
                    {actionLoading ? 'Đang xử lý...' : '✅ Duyệt cập nhật'}
                  </button>
                </>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col" style={{ maxHeight: '90vh' }}>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 bg-slate-800">
              <div className="flex items-center gap-4">
                <button
                  ...
                  className="flex items-center gap-2 text-slate-400 hover:text-white transition-colors font-medium bg-slate-700 hover:bg-slate-600 px-3 py-1.5 rounded-lg text-sm border border-slate-600"
                >
                  ← Quay lại
                </button>
              ...
              <button
                onClick={() => setPreviewLesson(null)}
                className="text-slate-400 hover:text-white transition-colors p-1.5 hover:bg-slate-700 rounded-lg"
              >✕</button>
            </div>
              ...
              {previewLesson.type === 'video' && (() => {
                ...
                return (
                  <div className="w-full max-w-3xl mx-auto">
                    {embed?.type === 'youtube' && (
                      <LMSVideoPlayer key={embed.id} videoId={embed.id} title={previewLesson.title} />
                    )}
                    {embed?.type === 'iframe' && (
                  ...
                  {previewLesson.text_content ? (
                    <div>
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{ __html: previewLesson.text_content }}
                      />
                    </div>
                  ) : (
                        ...
                        <div key={q._id || idx}>
                          ...
                          <div className="space-y-2.5">
                            {q.options?.map((optText, oIdx) => (
                              <label key={oIdx} className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-colors ${
                                oIdx === q.correctIndex
            ...
            <div className="px-5 py-3.5 border-t border-slate-800 bg-slate-800 flex justify-between items-center">
              ...
              <button
                onClick={() => setPreviewLesson(null)}
                className="px-5 py-2 text-sm font-medium text-white bg-slate-700 hover:bg-slate-600 rounded-xl transition-colors flex items-center gap-2 border border-slate-600"
              >
                ← Quay lại danh sách
              </button>
            </div>
