...
export default function DashboardLayout({ sidebarLinks, title }) {
    ...
    // Default sidebar links per role if not passed as prop
    const defaultLinks = user?.role === 'admin' ? [
        { path: '/admin/dashboard', label: 'Tổng quan', icon: <BookIcon className="w-4 h-4" /> },
        { path: '/admin/users', label: 'Người dùng', icon: <UserIcon className="w-4 h-4" /> },
    ] : [];

    const links = sidebarLinks ?? defaultLinks;
    const pageTitle = title ?? (user?.role === 'admin' ? 'Admin' : 'Dashboard');
